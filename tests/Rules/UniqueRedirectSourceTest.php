...
function uniquePasses(mixed $source, string $site, ?string $exceptId = null): bool
{
...
it('compares normalized paths', function () {
    Redirect::make()->source('/old')->destination('/new')->site('default')->save();

    expect(uniquePasses('/old/', 'default'))->toBeFalse()
        ->and(uniquePasses('/old?campaign=test', 'default'))->toBeFalse();
});

it('compares regex sources without changing their case', function () {
    Redirect::make()->source('#^/Blog$#')->destination('/new')->site('default')->save();

    expect(uniquePasses('#^/Blog$#', 'default'))->toBeFalse()
        ->and(uniquePasses('#^/blog$#', 'default'))->toBeTrue();
});

it('allows the same normalized source on another site', function () {
    Redirect::make()->source('/old')->destination('/new')->site('default')->save();

    expect(uniquePasses('/old/', 'french'))->toBeTrue();
});
