...
beforeEach(function () {
    ...
    $this->sitemap = new SitemapBuilder('test');
});
...
it('returns the site from the parent sitemap', function () {
    $this->sitemap->add('https://example.com/page');

    expect($this->sitemap->urls()->first()->site())->toBe('english');
});
...
it('converts to array with expected structure', function () {
    ...
    $this->sitemap->add('https://example.com/page', function (CustomSitemapUrl $url) {
        $url->changefreq('weekly')
            ->priority('0.8');
    });

    $array = $this->sitemap->urls()->first()->toArray();
...
it('can be fully configured via fluent setters', function () {
    $url = new CustomSitemapUrl($this->sitemap, 'https://example.com/page');

    $url->alternates([['href' => 'https://example.com/en/page', 'hreflang' => 'en']])
        ->lastmod(Carbon::parse('2025-01-01 00:00:00', 'UTC'))
        ->changefreq('daily')
        ->priority('0.7');

    expect($url->loc())->toBe('https://example.com/page')
        ->and($url->alternates())->toHaveCount(1)
        ->and($url->lastmod())->toStartWith('2025-01-01T00:00:00')
