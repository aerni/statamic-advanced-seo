...
it('enables a disabled redirect', function () {
    $redirect = tap(Redirects::make()->source('/old')->destination('/new')->site('default')->enabled(false))->save();

    $this->actingAs($this->super)
        ->post(cp_route('advanced-seo.redirects.enable', $redirect->id()))
        ->assertNoContent();

    expect(Redirects::find($redirect->id())->enabled())->toBeTrue();
});

it('disables an enabled redirect', function () {
    $redirect = tap(Redirects::make()->source('/old')->destination('/new')->site('default')->enabled(true))->save();

    $this->actingAs($this->super)
        ->post(cp_route('advanced-seo.redirects.disable', $redirect->id()))
        ->assertNoContent();

    expect(Redirects::find($redirect->id())->enabled())->toBeFalse();
});

it('forbids a viewer from enabling a redirect', function () {
    $redirect = tap(Redirects::make()->source('/old')->destination('/new')->site('default')->enabled(false))->save();

    $this->actingAs(redirectViewer())
        ->postJson(cp_route('advanced-seo.redirects.enable', $redirect->id()))
        ->assertForbidden();
});

it('forbids a viewer from disabling a redirect', function () {
    $redirect = tap(Redirects::make()->source('/old')->destination('/new')->site('default')->enabled(true))->save();

    $this->actingAs(redirectViewer())
        ->postJson(cp_route('advanced-seo.redirects.disable', $redirect->id()))
        ->assertForbidden();
});
