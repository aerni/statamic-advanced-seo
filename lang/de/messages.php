<?php

return [
    'site' => 'Website',
    'site_description' => 'Website-weite Einstellungen konfigurieren.',
    'site_defaults' => 'Website-Standards',

    'collections' => 'Sammlungen',
    'collections_description' => 'Sammlungen konfigurieren und Standardwerte für Einträge festlegen.',

    'taxonomies' => 'Taxonomien',
    'taxonomies_description' => 'Taxonomien konfigurieren und Standardwerte für Begriffe festlegen.',

    'reset_to_default' => 'Auf Standard zurücksetzen',

    'entry' => 'Eintrag',
    'entries' => 'Einträge',
    'term' => 'Begriff',
    'terms' => 'Begriffe',

    'seo_set_localization_saved' => 'SEO-Standards gespeichert',
    'seo_set_localization_deleted' => 'SEO-Standards gelöscht',
    'seo_set_config_saved' => 'SEO-Konfiguration gespeichert',
    'seo_set_config_deleted' => 'SEO-Konfiguration gelöscht',

    'origins' => 'Ursprünge',
    'origins_instructions' => 'Wähle aus, um Werte von ausgewählten Ursprüngen zu übernehmen.',
    'configure_title' => ':title konfigurieren',
    'no_items_for_site' => 'Keine :title für die ausgewählte Website konfiguriert.',
    'disable_confirmation' => 'Möchtest du dieses Element wirklich deaktivieren? Alle SEO-Daten werden gelöscht.',
    'token_picker_placeholder' => 'Tippe / um ein Token hinzuzufügen',
    'token_suggestion_placeholder' => 'Tippe um zu suchen …',
    'no_results' => 'Keine Ergebnisse',
    'social_image_updates_on_save' => 'Das Bild wird beim Speichern aktualisiert.',
    'social_image_generates_on_first_save' => 'Das Bild wird beim ersten Speichern generiert.',
    'from_domain' => 'Von :domain',

    'overridden_by_site_defaults_badge' => 'Überschrieben',
    'overridden_by_site_defaults_tooltip' => 'Dieser Wert wird von den Website-Standards überschrieben und wird erst wirksam, wenn der Website-Standardwert entfernt wird.',

    'ai_generate' => 'Mit KI generieren',
    'ai_insufficient_content' => 'Füge mindestens :characters weitere Zeichen hinzu, um mit KI zu generieren.',
    'ai_generation_failed' => 'KI-Generierung fehlgeschlagen. Bitte versuche es erneut.',

    'upgrade_prompt' => 'Du verwendest Advanced SEO Free. <a href=":url" target="_blank" class=":classes">Upgrade auf Pro</a> für Sitemaps, Multi-Site, KI, Berechtigungen und mehr.',

    'pro_features' => 'Upgrade auf Pro',
    'pro_features_instructions' => '<a href="https://statamic.com/addons/aerni/advanced-seo" target="_blank" class="underline">Pro holen</a> um diese und weitere Features freizuschalten.',
    'pro_feature_sitemaps' => 'Sitemaps',
    'pro_feature_sitemaps_description' => 'XML-Sitemaps für Collections, Taxonomien und benutzerdefinierte Routen.',
    'pro_feature_multi_site' => 'Multi-Site',
    'pro_feature_multi_site_description' => 'Lokalisierte SEO-Standardwerte mit Origin-Vererbung.',
    'pro_feature_ai' => 'KI-Inhaltsgenerierung',
    'pro_feature_ai_description' => 'Meta-Titel und -Beschreibungen mit KI generieren.',
    'pro_feature_social_images' => 'Social Images',
    'pro_feature_social_images_description' => 'Automatische Generierung von Social-Media-Bildern mit anpassbaren Themes.',
    'pro_feature_permissions' => 'Berechtigungen',
    'pro_feature_permissions_description' => 'Granulare Kontrolle darüber, wer SEO konfigurieren und bearbeiten darf.',
    'pro_feature_graphql' => 'GraphQL',
    'pro_feature_graphql_description' => 'Vollständige GraphQL-Unterstützung für Headless-Setups.',
    'pro_feature_eloquent' => 'Eloquent Driver',
    'pro_feature_eloquent_description' => 'SEO-Daten in der Datenbank statt in Dateien speichern.',
    'pro_feature_custom_routes' => 'Benutzerdefinierte Routen',
    'pro_feature_custom_routes_description' => 'SEO-Metadaten für benutzerdefinierte Statamic- und Laravel-Routen.',
    'pro_feature_custom_tokens' => 'Benutzerdefinierte Tokens',
    'pro_feature_custom_tokens_description' => 'Eigene Normalizer und Value-Tokens für Titel- und Beschreibungsfelder registrieren.',
];
