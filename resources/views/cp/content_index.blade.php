            ...
            @if (Statamic\Facades\Collection::all()->count() > 0)
                <div class="w-full px-2 mb-4 lg:w-1/2 widget">
                    <div class="p-0 card content">
                        ...
                        <div class="p-2">
                            @foreach (Statamic\Facades\Collection::all()->filter(fn ($collection) => ! in_array($collection->handle(), config('advanced-seo.disabled.collections', [])))->sort() as $collection)
            ...
            @if (Statamic\Facades\Taxonomy::all()->count() > 0)
                <div class="w-full px-2 mb-4 lg:w-1/2 widget">
                    <div class="p-0 card content">
                        ...
                        <div class="p-2">
                            @foreach (Statamic\Facades\Taxonomy::all()->filter(fn ($taxonomy) => ! in_array($taxonomy->handle(), config('advanced-seo.disabled.taxonomies', [])))->sort() as $taxonomy)
