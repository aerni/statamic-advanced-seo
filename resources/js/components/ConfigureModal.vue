...
<template>
	<Modal
		:title="__('advanced-seo::messages.configure_seo_defaults')"
		:open
		:dismissable="!busy"
		@dismissed="close"
	...
	>
		<div
			v-if="!sites"
			class="absolute inset-0 flex items-center justify-center bg-white bg-opacity-75 pointer-events-none select-none dark:bg-dark-700"
		>
			<Icon name="loading" />
		</div>
		...
		<template #footer>
			<div class="flex items-center justify-end pt-3 pb-1 space-x-3">
				<Button
					variant="ghost"
					:disabled="busy"
					:text="__('Cancel')"
