...
<template>
    <Card class="max-w-[600px]">
        <div class="flex items-center gap-3 mb-1.5">
            <div class="flex items-center justify-center rounded-full size-[26px] shrink-0 bg-[#f3f5f6] border border-[#d2d2d2]">
            ...
            <div class="min-w-0">
                <div v-if="siteName" class="text-[14px] leading-[20px] text-gray-800 dark:text-gray-300">{{ siteName }}</div>
                <div v-else class="flex items-center h-[20px]"><div class="h-2.5 w-24 bg-gray-100 dark:bg-gray-700 rounded" /></div>
                <div class="text-[12px] leading-[18px] text-gray-600 dark:text-gray-400 truncate">
                    <template v-for="(breadcrumb, i) in breadcrumbs" :key="i">
                        ...
                        <span v-else class="inline-block w-12 h-2 align-middle bg-gray-100 rounded dark:bg-gray-700" />
                    </template>
                </div>
            </div>
        </div>
        <div class="text-[20px] leading-[26px] text-[#1a0dab] dark:text-[#8ab4f8] mb-1 line-clamp-2">
            ...
            <template v-else>{{ titlePrefix }}<span class="inline-block w-32 h-5 align-middle bg-gray-100 rounded dark:bg-gray-700" />{{ titleSuffix }}</template>
        </div>
        <div v-if="description" class="text-sm leading-[22px] text-gray-600 dark:text-gray-400 line-clamp-2">{{ description }}</div>
        <div v-else>
            <div class="flex items-center h-[22px]"><span class="w-full h-3 bg-gray-100 rounded dark:bg-gray-700" /></div>
            <div class="flex items-center h-[22px]"><span class="w-2/3 h-3 bg-gray-100 rounded dark:bg-gray-700" /></div>
        </div>
    </Card>
</template>
